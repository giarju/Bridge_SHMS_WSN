...
import com.virtenio.preon32.node.Node12;
...
public class RadioInit {
	...
	public static AT86RF231 initRadio() throws Exception {
		...
		AT86RF231 radio = Node.getInstance().getTransceiver(9);
		radio.open();
		radio.reset();
...
//		final CPU cpu = NativeCPU.getInstance();
		...
		AsyncEvent event2 = events.getEvent(9);
		event2.addHandler(new AsyncEventHandler() {
			@Override
			public void handleAsyncEvent(int eventId) {
				try {
//					cpu.set(CPUConstants.K_POWER_STATE, CPUConstants.V_POWER_STATE_IDLE);
					CPUHelper.setPowerState(CPUConstants.V_POWER_STATE_IDLE, 6);
				} catch (CPUException e) {
					e.printStackTrace();
				}
			}
		});
		NativeIRQ irq = NativeIRQ.getInstance(Node12.RADIO_IRQ_ID);
		irq.open(IRQ.MODE_ANY_CHANGE, 9);
		irq.enable();
		event2.bindTo(9);
	...
	public static AT86RF231 sleepRadio() throws Exception {
		AT86RF231 radio = Node.getInstance().getTransceiver();
		radio.open();
		radio.reset();
		radio.setState(AT86RF231.STATE_SLEEP);
		return radio;
	}

	public static AT86RF231 closeRadio() throws Exception {
		AT86RF231 radio = Node.getInstance().getTransceiver();
		radio.reset();
		radio.close();
		return radio;
